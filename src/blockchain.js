...
class Blockchain {
  ...
  createGenesis() {
    const header = new Header(Date.now());
    const block = new Block(header, "Genesis block", 0);
    header.merkleRoot = SHA256(block.transactions).toString();
    header.difficulty = 0;
    return block;
  }
  ...
  mineMemPool(minerAddress, memPool) {
    ...
    const block = new Block(header, tempMemPool, tempMemPool.length);
  ...
  adjustDifficulty() {
    // if it was too easy/hard to mine a block (according to what MineRate we specify), then adjust difficulty
    if (this.getLatestBlock().header.timeStamp + MINE_DIFFICULTY_RATE > Date.now()) {
      this.difficulty += 1;
    } else if (
      this.getLatestBlock().header.timeStamp + MINE_DIFFICULTY_RATE >
      Date.now() + MINE_DIFFICULTY_RATE
    ) {
      this.difficulty -= 1;
    }
  }
  ...
  isChainValid() {
    ...
    for (let i = 1; i < this.chain.length; i++) {
      ...
      if (!currentBlock.hasValidTransactions()) {
        throw new Error(
          "Chain have manipulated transactions. Chain not valid!"
        );
      }
